...
The configuration files contain key/value pairs, with one key and one value per line. For each key/value pair, follow these rules:
...
The values for all supported keys are defined below.

The definition for each key includes one of the following designations:

* *Required*: Required for every node
* *Best practice*: Required for certain node types
* *Optional*: Optional for all nodes

*ADMIN_IP* (Best practice)

Value:: Grid Network IPv4 address of the primary Admin Node for the grid to which this node belongs. Use the same value you specified for GRID_NETWORK_IP for the grid node with NODE_TYPE = VM_Admin_Node and ADMIN_ROLE = Primary. If you omit this parameter, the node attempts to discover a primary Admin Node using mDNS.

See also: link:how-grid-nodes-discover-primary-admin-node.html[How grid nodes discover the primary Admin Node]

NOTE: This value is ignored, and might be prohibited, on the primary Admin Node.

*ADMIN_NETWORK_CONFIG* (Optional)

Value:: DHCP, STATIC, or DISABLED

*ADMIN_NETWORK_ESL* (Optional)

Value:: Comma-separated list of subnets in CIDR notation to which this node should communicate using the Admin Network gateway.

Example: `172.16.0.0/21,172.17.0.0/21`

*ADMIN_NETWORK_GATEWAY* (Optional/Required)

Value:: IPv4 address of the local Admin Network gateway for this node. Must be on the subnet defined by ADMIN_NETWORK_IP and ADMIN_NETWORK_MASK. This value is ignored for DHCP-configured networks.
...
*ADMIN_NETWORK_IP* (Optional)

Value:: IPv4 address of this node on the Admin Network. This key is only required when ADMIN_NETWORK_CONFIG = STATIC; don't specify it for other values.
...
aINTERFACES_TARGET_nnnn
aOptional
aName and optional description for an extra interface you want to add to this node. You can add multiple extra interfaces to each node.
