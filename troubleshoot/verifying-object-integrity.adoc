...
You create and run one object existence check job at a time. When you create a job, you select the  Storage Nodes and volumes you want to verify. You also select the consistency setting for the job.
...
An object existence check job can take days or weeks to complete, depending on the number of objects in the grid, the selected storage nodes and volumes, and the selected consistency setting. You can run only one job at a time, but you can select multiple Storage Nodes and volumes at the same time.
...
.	Select *Continue*.
.	Select the consistency setting for the job.
+
The consistency setting determines how many copies of object metadata are used for the object existence check.
+
...
+
For more information about consistency settings, see the descriptions in the wizard.
+
. Select *Continue*.
...
.	If you selected the strong-site or strong-global consistency setting for the job, wait approximately three weeks for metadata consistency and then rerun the job on the same volumes again.
+
...
+
.. In the Rerun jobs wizard, review the selected nodes and volumes and the consistency setting.
.. When you are ready to rerun the jobs, select *Rerun*.

The Active job tab appears. All the jobs you selected are rerun as one job at a consistency setting of strong-site. A *Related jobs* field in the Details section lists the job IDs for the original jobs.
