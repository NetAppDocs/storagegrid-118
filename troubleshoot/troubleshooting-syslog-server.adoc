...
These errors are shown by the Configure external syslog server wizard if you have problems sending test messages to validate that the external syslog server is correctly configured. They might also appear as part of a runtime syslog configuration alert.
