...
* To control remote IPMI access to the BMC in Grid Manager, go to *CONFIGURATION* > *Security settings* > *Appliances*:
...
* To control remote IPMI access to the BMC the Grid management API, use the management API private endpoint: `PUT /private/bmc`.
...
* If the storage controller in the appliance includes FDE or FIPS drives and the Drive Security feature is enabled, use SANtricity to configure Drive Security keys. See https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/accessing-and-configuring-santricity-system-manager.html[Configure SANtricity System Manager (SG6000 and SG5700)^].
* For appliances without FDE or FIPS drives, enable node encryption using a Key Management Server (KMS). See https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/optional-enabling-node-encryption.html[Optional: Enable node encryption^].