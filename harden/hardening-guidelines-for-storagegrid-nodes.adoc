...
* To control remote IPMI access to the BMC the Grid management API, use the private endpoint: `PUT /private/bmc`.
...
* For appliance models containing SED, FDE, or FIPS NL-SAS hard drives that you manage with SANtricity System Manager, https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/accessing-and-configuring-santricity-system-manager.html[enable and configure SANtricity Drive Security^].

* For appliance models containing SED or FIPS NVMe SSDs that you manage using the StorageGRID Appliance Installer and Grid Manager, https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/optional-enabling-node-encryption.html[enable and configure StorageGRID drive encryption^].

* For appliances without SED, FDE, or FIPS drives, enable and configure StorageGRID software node encryption https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/optional-enabling-node-encryption.html[using a Key Management Server (KMS)^].
