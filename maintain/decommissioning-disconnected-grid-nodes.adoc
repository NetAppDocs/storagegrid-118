...
+
IMPORTANT: Be careful when decommissioning more than one disconnected grid node at a time, especially if you are selecting multiple disconnected Storage Nodes.
...
+
IMPORTANT: If you believe that object data can still be recovered from the node, don't perform this procedure. Instead, contact technical support to determine if node recovery is possible.
...
Before decommissioning a disconnected *Archive Node*, confirm that the Archive Node is no longer being used in any rules in any active ILM policy:
...
. If any ILM rules are listed, look at the *Used in active policy* column to determine if the Archive Node storage pool is being used.
. If the storage pool is being used, create a new ILM policy that no longer uses that rule.
...
+
IMPORTANT: Don't perform this step until the nodes have shut down automatically.
