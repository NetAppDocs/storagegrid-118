...
* In rare cases, such as when instructed by technical support, or when you know that the replacement node has fewer volumes available for object storage than the original node, you must link:restoring-object-data-to-storage-volume.html[restore object data manually] using the `repair-data` script. If one of these cases applies, answer `n`.
+
[NOTE]
====
If you answer `n` to restore object data manually:
