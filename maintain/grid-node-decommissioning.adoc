...
== Types of decommission procedures
...
Before performing either procedure, review the considerations for each  each type of node:
...
CAUTION: Don't remove a grid node's virtual machine or other resources until instructed to do so in decommission procedures.
