...
If an ILM policy refers to the site you want to remove, you must remove the policy, edit it, or replace it with a new policy that meets these requirements:
...
+
If you're editing a policy, follow these guidelines:
...
* Select storage pools that don't refer to the site.
+
...
+
The ILM policy details page for the policy appears in a new browser tab. The Decommission Site page will remain open on the other tab.
.. Clone the policy, then link:../ilm/creating-ilm-policy.html[follow the steps for creating an ILM policy] and these guidelines:
