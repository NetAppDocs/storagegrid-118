...
IMPORTANT: StorageGRID is a distributed system composed of multiple nodes working with each other. Don't use disk snapshots to restore grid nodes. Instead, refer to the recovery and maintenance procedures for each type of node.
...
* The StorageGRID Appliance Installer version on the replacement appliance matches the software version of your StorageGRID system, as described in https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/verifying-and-upgrading-storagegrid-appliance-installer-version.html[Verify and upgrade StorageGRID Appliance Installer version^].
