...
To install StorageGRID on Red Hat Enterprise Linux (RHEL), you must install some third-party software packages. Some supported operating systems don't contain these packages by default. The software package versions that StorageGRID installations are tested on include those listed on this page. 

NOTE: If you select a base operating system and container runtime installation option that requires any of these packages, and they are not already present in your operating system installation, install one of the versions listed here if available from your operating system vendor. Otherwise, use the default package versions available from your vendor.
