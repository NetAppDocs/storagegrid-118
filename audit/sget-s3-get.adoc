...
| LITY
| List Type 2
| Present if list-type is specifed to be 2. For GET bucket (listBucket) operations only.

| NCHD
| Number of Children
| Includes keys and common prefixes. For GET bucket (listBucket) operations only.
...
| Whether the returned content is truncated. For GET bucket (listBucket) operations only.
