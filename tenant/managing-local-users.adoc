...
** As needed, a banner message indicates if users were not cloned to the tenant on the destination grid. In this case, you can select *Retry cloning* to <<clone-users,try cloning again>>.
...
.	In the message banner, select Retry cloning. The cloning operation is retried.
.	Alternatively, you can view a list of all users that weren't cloned. From the Users page, select each user that indicates _(Cloning failed)_ below the user name. Select *Actions* > *Clone users*.

You can view the status of the clone operation from the details page of each user you're cloning.
