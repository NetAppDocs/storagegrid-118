...
+
You can <<clone-users,clone new users groups or retry a user clone>> that failed.
...
To clone new users or retry a clone that failed:

. Select the checkbox for the users you want to clone.
. Select *Actions* > *Clone users*.
+
You can view the status of the clone operation from the details page of the user you're cloning.
. If a "clone failed" message appears, make corrections and try again.
