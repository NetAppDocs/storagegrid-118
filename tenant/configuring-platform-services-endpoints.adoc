...
For Kafka endpoints, Mutual TLS is not supported. As a result, if you have `ssl.client.auth` set to `required` in your Kafka server, it might cause Kafka endpoint configuration issues.
