...
* If a bucket has object versioning enabled, any delete markers that were created in StorageGRID 11.7 or earlier that are in the bucket when you start these steps will not be removed by the delete objects operation. If you want to delete a versioned bucket created in StorageGRID 11.7 or earlier after all objects have been deleted, you must remove any pre-existing delete markers. 
