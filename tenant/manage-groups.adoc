...
+
You can <<clone-groups,clone new groups or retry a group clone>> that failed.
...
.	Alternatively, you can view a list of all groups that weren't cloned. From the Groups page, select each group that indicates _(Cloning failed)_ below the group name. Select *Actions* > *Clone groups*.

You can view the status of the clone operation from the details page of each group you're cloning.
