...
<NotificationConfiguration>
  <TopicConfiguration>
    ...
    <Topic>arn:aws:kafka:us-east-1:050340950352:sgws-topic</Topic>
    <Event>s3:ObjectCreated:*</Event>
   </TopicConfiguration>
 </NotificationConfiguration>
----
...
+
The following example shows an XML with the URN of an Amazon SNS endpoint notification.
...
+
For example, if your destination topic is hosted on the Amazon SNS or Kafka topic, you could configure the service to send you an email when the notification is delivered.
+
[role="tabbed-block"]
====
