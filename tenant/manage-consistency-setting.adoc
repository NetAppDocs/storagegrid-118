...
---
= Manage bucket consistency settings
:icons: font
:imagesdir: ../media/
...
== How bucket settings affect consistency

Buckets have multiple settings that affect the behavior of the buckets and the objects within those buckets. 

The following bucket settings use *strong* consistency by default. If two or more Storage Nodes are not available within any site, or if a site is not available, any changes to these settings might not take effect for some time.
...
== Change bucket consistency setting

To specify a different consistency value when changing a bucket setting:
