...
* Configure and manage bucket options, such as link:changing-consistency-level.html[consistency control], link:enabling-or-disabling-last-access-time-updates.html[last access time updates], link:changing-bucket-versioning.html[object versioning], link:using-s3-object-lock.html[S3 Object Lock] and link:update-default-retention-settings.html[default bucket retention]
