...
a|
The authentication method used when creating Kafka endpoints requires the following credentials, which are different to those used for other endpoints:

* SASL/PLAIN
* SASL/SCRAM-SHA-256
* SASL/SCRAM-SHA-512

You should also note that Kafka endpoints do not support storage proxy settings.
