...
.. Optionally, change the setting of the *crossGridReplication* audit category to manage the volume of audit messages:
+
* If *Error* (default), Cross-grid replication requests and PUT audit messages are not included in the audit output.
+
* If *Normal*, Cross-grid replication requests and PUT audit messages are included which significantly increases the volume of the audit output.
