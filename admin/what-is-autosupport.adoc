...
AutoSupoport XML file contents
...
| AutoSupport status file. Provides details of protocol used, URL, support address, polling interval, and OnDemand AutoSupport if enabled or disabled.
...
| Statistics based on the bucket configuration. Provides bucket configuration details that can give you insight into the configurations that are set at the bucket level. Platform services, compliance, and bucket consistency level are examples of bucket configurations.
...
Table Name
| Grid-wide configuration information file. Contains info about grid certificates, metadata reserved space, grid-wide configuration settings (compliance, S3 Object Lock, object compression, alerts, syslog, ILM configuration), EC profile details, DNS name, NMS name, and more.
...
| LOG.TGZ
| 
| Downloadable log file. Contains log files from all nodes.
...
The status of this data item

The description of the error

The AutoSupport content type for this data
| Contains the  AutoSupport metadata and brief descriptions of all the AutoSupport XML files.
...
Table name
| Server configurations and events file. Contains platform type, operating system, installed memory, available memory, storage connectivity, storage appliance chassis serial number, storage controller failed drive count, compute controller chassis temperature, compute hardware, compute controller serial number, power supply, drive size, drive type, and more for each node.
...
Table name
| Service node information file. Contains details such as allocated table space, free table space, reaper metrics of the database, segment repair duration, repair ob duration, auto job restarts, auto job aborts, and more.
...
Table name
| High-level system status data that summarizes StorageGRID usage information. Provides details such as name of grid, names of sites, number of Storage Nodes per grid, number of Storage Nodes per site, license type, license capacity and usage, software support terms, and details of S3 and Swift operations.
...
Other labels
| Current system alerts that indicate any potential problems in the StorageGRID system.
