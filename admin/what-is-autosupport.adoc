...
summary: 'The AutoSupport feature enables StorageGRID to send health and status packages to the NetApp Support Site.'
---
...
[.lead]
The AutoSupport feature enables StorageGRID to send health and status packages to the NetApp Support Site.
...
By default, AutoSupport is enabled when you first install StorageGRID. You can link:configure-autosupport-grid-manager.html[change the default AutoSupport configuration] if needed.

NOTE: If AutoSupport is not enabled, a message appears on the Grid Manager dashboard. The message includes a link to the AutoSupport configuration page. If you close the message, it will not appear again until your browser cache is cleared, even if AutoSupport remains disabled.

== Information included in AutoSupport packages

AutoSupport packages include information such as the following:
...
For more information about the details included in an AutoSupport package, expand *AutoSupport package content*:
