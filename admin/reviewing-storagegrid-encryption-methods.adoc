...
| You are prompted to set a password if FIPS drives are detected and a password has not yet been set. Before starting installation, go to *Configure Hardware* > *Node Encryption* to set an SSD key and passphrase that applies to all StorageGRID-managed self-encrypting drives in a node.

If you don't want to use drive encryption, enable drive encryption and then disable it without the `eraseData` option.
| Storage appliances that have FIPS drives. All data on the secured drives is protected against physical loss or removal from the data center. Can't be used with some storage appliances or with any service appliances.

Guidelines are provided for configuring encryption for StorageGRID appliances in SANtricity System Manager.
