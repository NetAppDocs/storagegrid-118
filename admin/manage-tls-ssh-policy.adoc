...
*Note*: If you select this policy, all nodes must be link:../maintain/rolling-reboot-procedure.html[rebooted in a rolling fashion] to apply the configuration on the internal FIPS module. Use *Maintenance* > *Rolling reboot* to initiate and monitor reboots.
