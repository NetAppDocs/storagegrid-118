...
summary: 'Managing a key management server \(KMS\) involves viewing or editing details, viewing encrypted nodes, and removing a KMS when it is no longer needed.'
---
= Manage a KMS
:icons: font
:imagesdir: ../media/
...
[.lead]
Managing a key management server (KMS) involves viewing or editing details, viewing encrypted nodes, and removing a KMS when it is no longer needed.
...
* You are signed in to the Grid Manager using a link:../admin/web-browser-requirements.html[supported web browser].
* To make changes to KMS details, you have the link:admin-group-permissions.html[Root access permission].
...
. If the Certificate expiration column indicates that a certificate has expired or is nearing expiration, select *Edit certificate* to replace a certificate with a new one.
+
