...
. As often as required by your organization's security practices, select *Rotate key* to create a new version of the key.
+
...
[CAUTION]
====
Never attempt to rotate a key by changing the key name (alias) for the KMS. Instead, rotate the key by updating the key version in the KMS software, or select *Rotate key* on the KMS details page. StorageGRID requires all previously used key versions (as well as any future ones) to be accessible from the KMS with the same key alias. If you change the key alias for a configured KMS, StorageGRID might not be able to decrypt your data.
