...
* If you choose the option to clone existing local users and groups, the local users and groups of the source tenant will be cloned to the newly cloned tenant. If you want, you can stop the cloning in progress or retry cloning if the last clone operation wasn't completed.
...
* If you want to re-enable this permission, delete this tenant on the destination grid first; otherwise, re-enabling this permission will result in an error.

NOTE: Enabling the *Use grid federation connection* permission (either at create time or at edit time) makes the current grid the source grid and triggers cloning to the replica grid specified by the grid federation connection you select. If you re-enable this permission on a replica grid, its role changes from destination grid to source grid.
