...
If you need to install an encrypted appliance into another StorageGRID system, you must first decommission the grid node to move object data to another node. Then, you can use the StorageGRID Appliance Installer to https://review.docs.netapp.com/us-en/storagegrid-appliances_main/commonhardware/monitoring-node-encryption-in-maintenance-mode.html[clear the KMS configuration^]. Clearing the KMS configuration disables the *Node Encryption* setting and removes the association between the appliance node and the KMS configuration for the StorageGRID site.
