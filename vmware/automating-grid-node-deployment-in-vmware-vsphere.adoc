...
+ 
TIP: If all or most of the nodes use the same type of temporary installation password, specify the type in the global parameter section. Then, optionally use a different setting for individual nodes. For example, if you select *Use Custom Password* globally, you can use *CUSTOM_TEMPORARY_PASSWORD=<password>* to set a password for each node.
+
...
**	*Use custom password*: The value provided with *CUSTOM_TEMPORARY_PASSWORD=<password>* is used as the temporary installation password. 
+
...
Optional. The temporary password to use when accessing this VM and using SSH during installation. Ignored if *TEMPORARY_PASSWORD_TYPE* is set to *Use node name* or *Disable password*.
