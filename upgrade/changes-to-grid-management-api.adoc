...
== `ilm-policies-v4` API removed

The removal of the `ilm-policies-v4` API results in the following changes:
...
* The optional type query parameter now takes values `inactive` and `active.` The previous values in v3 were `proposed`, `active`, and `historical`.