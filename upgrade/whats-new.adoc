...
You can now change the default link:../s3/consistency-controls.html[consistency control]. To make the change, use the grid-config endpoint of the link:../admin/using-grid-management-api.html[Grid Management private API]. The new default will be applied to buckets created after the change. To determine when the default was changed, consult the audit logs.  
