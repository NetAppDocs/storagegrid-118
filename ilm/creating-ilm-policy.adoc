...
When you create an ILM policy, you start by selecting and arranging the ILM rules. Then, you verify the behavior of your proposed policy by simulating it against previously ingested objects. When you are satisfied that the proposed policy is functioning as intended, you can activate it.

To provide different quality-of-service options to tenants, you can have more than one active policy at a time. You use ILM policy tags that identify each policy, then tag a bucket to apply a policy to the bucket. 
...
An active ILM policy controls the placement, duration, and data protection of all objects.
...
Every StorageGRID system must have at least one active ILM policy. If you have more than one active ILM policy, you create a tag for each policy and assign a policy to each tag. You then apply tags to buckets. A default tag is automatically applied to a bucket if no other tag is assigned.

A StorageGRID system might also have one proposed ILM policy and any number of historical policies.

When you first create an ILM policy, you create a proposed policy by selecting one or more ILM rules and arranging them in a specific order. After you have simulated the proposed policy to confirm its behavior, you activate it.
...
If you activate more than one ILM policy at a time and apply policy tags to buckets, the objects in each bucket are managed according to the policy assigned to the tag.

Historical ILM policies can't be deleted.
