...
. If you are creating more than one ILM policy, link:use-ilm-policy-tags.html[create ILM policy tags] and apply a policy to each tag.
...
. Repeat these steps to create additional ILM policies. If you create more than one policy, link:use-ilm-policy-tags.html[create ILM policy tags and assign a policy to each tag].
+
NOTE: You must use tags before you can activate multiple policies.
