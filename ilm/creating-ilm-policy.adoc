...
In case you want to have multiple active policies at the same time, a link:use-ilm-policy-tags.html[default ILM policy tag] is automatically created when you install StorageGRID.
...
Every StorageGRID system must have at least one active ILM policy. If you have more than one active ILM policy, you create a link:use-ilm-policy-tags.html[tag for each policy] and assign a policy to each tag. Tenants then apply tags to buckets or containers. A default tag is automatically applied to a bucket (container) if no other tag is assigned.
