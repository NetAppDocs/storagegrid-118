...
== How ILM rules and consistency settings interact to affect data protection

Both your ILM rule and your choice of consistency setting affect how objects are protected. These settings can interact.

For example, the ingest behavior selected for an ILM rule affects the initial placement of object copies, while the consistency setting used when an object is stored affects the initial placement of object metadata. Because StorageGRID requires access to both an object's data and metadata to fulfill client requests, selecting matching levels of protection for the consistency setting and ingest behavior can provide better initial data protection and more predictable system responses.

Here is a brief summary of the consistency settings that are available in StorageGRID:
...
NOTE: Before selecting a consistency setting, link:../s3/consistency-controls.html[read the full description of consistency settings]. You should understand the benefits and limitations before changing the default value.
