 ...
 * The default rule should create replicated copies.
+
NOTE: Don't use a rule that creates erasure-coded copies as the default rule for a policy. Erasure-coding rules should include the *Object size (MB) greater than 0.2* advanced filter to prevent smaller objects from being erasure-coded.
