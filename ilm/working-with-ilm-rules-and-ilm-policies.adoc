...
//if a rule in your "snapshot" no longer exists or has been edited, verify that the edits work for your current situation. banner will appear 
...
//+
//NOTE: If you remove a rule from a policy that is inactive, the image:../media/icon_ilm_rule_historical.png[Icon ILM Rule Removed] icon appears for the rule when you view the policy details.

//snapshot of policy at a given time
//the clock icon means the rule that you're viewing in the historical snapshot has been removed/edited
//might inactivate, delete, edit policy
