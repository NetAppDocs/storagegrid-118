...
You can't edit a rule if it is being used in an active ILM policy. Instead, you can clone a rule and make any required changes to the cloned copy. Then, if required, you can remove the original rule from the policy and replace it with the modified version. You can't clone an ILM rule if it was created using StorageGRID version 10.2 or earlier.
...
You can't edit a rule if it is being used in an active ILM policy. Instead, you can clone these rules and make any required changes to the cloned copy. You also can't edit the system-provided rule, Make 2 Copies.
