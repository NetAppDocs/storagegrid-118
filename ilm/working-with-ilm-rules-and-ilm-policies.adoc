...
You can't edit a rule if it is used in any ILM policy. Instead, you can <<clone-ilm-rule,clone the rule>> and make any required changes to the cloned copy. Also, you can only edit the system-provided rules, Make 2 Copies (for a single-site grid) or 1 Copy Per Site (for a multi-site grid) for a new installation of StorageGRID 11.8 or later.
...
. Remove the ILM rule from the policy or remove the policy.
. On the ILM rules page, select the checkbox for the rule, and then select *Actions* > *Remove*.
