...
+
NOTE: ILM cannot be used to remove expired object delete markers or delete markers created in StorageGRID 11.7 or earlier. Use an S3 client request with a version ID specified or bucket lifecycle rules to remove expired object delete markers in StorageGRID 11.8 or later. Delete markers created in StorageGRID 11.7 or earlier can only be removed through S3 client requests.
