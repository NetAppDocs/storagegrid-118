...
To remove delete markers from a bucket that was created in StorageGRID 11.7 or earlier, the S3 client must explicitly delete the object version. Delete markers in buckets created in StorageGRID 11.7 or earlier aren't removed by ILM, bucket lifecycle rules, or Delete objects in bucket operations, even if the delete markers were created in StorageGRID 11.8 or later. 
