...
When a versioned object is deleted, StorageGRID creates a zero-byte delete marker as the current version of the object. 

Delete markers from a bucket that were created in StorageGRID 11.8 or later can be removed by ILM, bucket lifecycle rules, Delete objects in bucket operations, or an explicit S3 client deletion.

To remove delete markers from a bucket that were created in StorageGRID 11.7 or earlier, the S3 client must explicitly delete the object version. These delete markers aren't removed by ILM, bucket lifecycle rules, or Delete objects in bucket operations. 
