...
Delete markers created in StorageGRID 11.7 or earlier can only be removed through S3 client requests. They must be removed using S3 client requests before a versioned bucket can be deleted. Delete markers created in StorageGRID 11.7 or earlier are not removed by ILM, bucket lifecycle rules, or Delete objects in bucket operations. This restriction also applies to delete markers in buckets created in StorageGRID 11.7 or earlier even if these delete markers were in created in StorageGRID 11.8 or later.
