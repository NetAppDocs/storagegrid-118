...
+
NOTE: ILM cannot be used to remove expired object delete markers created in StorageGRID 11.7 or earlier. Use an S3 client request or bucket lifecycle to remove these expired object delete markers.
...
** Delete markers created in 11.7 or earlier can only be removed through S3 client requests, they are not removed by ILM, bucket lifecycle rules, or Delete objects in bucket operations.
