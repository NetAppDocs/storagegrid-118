...
The following diagrams illustrate the following concepts:

* A single policy with filters for tenants and buckets applied to the rules in the policy.
* Multiple policies assigned to tags that tenants can apply to buckets.

The second diagram illustrates how tenants can easily apply different tags to each bucket, and how a grid administrator can assign a different policy to a tag. These changes are illustrated by the dashed lines.
