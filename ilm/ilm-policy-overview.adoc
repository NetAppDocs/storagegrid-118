...
+
If your grid will use a simple data protection scheme with few tenant-specific and bucket-specific rules, use a single active ILM policy.
...
+ 
If you want to allow tenants to easily switch between multiple data protection policies on a per-bucket basis, use multiple ILM policies with policy tags.
...
The following diagram illustrates the difference between the ILM policy feature for StorageGRID 11.7 and earlier and StorageGRID 11.8. In particular, it shows:

* A single policy (before StorageGRID 11.8) with filters for tenants and buckets applied to the rules in the policy.
* Multiple policies (StorageGRID 11.8) assigned to tags that tenants can apply to buckets. Tenants can easily apply different tags to each bucket, as illustrated by the dashed lines. Those buckets would then be managed by policies other than those originally assigned.
...
If the default policy does not meet your storage requirements, you can create your own rules and policy. See link:what-ilm-rule-is.html[Create an ILM rule] and link:creating-ilm-policy.html[Create an ILM policy]. Each policy is assigned to a tag. Tenants can have their buckets managed by the appropriate policy by applying a tag to their buckets.
