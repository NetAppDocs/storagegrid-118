...
| In an ILM rule that is in a policy that was activated and later deactivated
| 
. Edit or delete the rule. If you edit the rule, remove all placements that use the erasure-coding profile.
. Continue with this procedure.
| link:working-with-ilm-rules-and-ilm-policies.html[Work with ILM rules and ILM policies]
|===
