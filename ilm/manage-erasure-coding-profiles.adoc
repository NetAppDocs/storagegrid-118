...
Status:: Whether the profile is activated or deactivated.
Storage pool:: The storage pool used for the profile.
Scheme:: The erasure-coding scheme used for the profile.
Sites:: The number of sites included in the storage pool.
Storage overhead:: The additional storage required for parity fragments relative to the object's data size, based on the erasure-coding scheme.
Storage Node redundancy:: The number of Storage Nodes that can be lost while still maintaining the ability to retrieve erasure coding objects.
Site redundancy:: Whether the erasure-coding scheme allows the object to be retrieved if a site is lost.
