 ...
 ** *Storage Type* (Storage Nodes only): Specify that a new Storage Node be used exclusively for metadata. The options are *Objects and metadata* and *Metadata only*. See link:../primer/what-storage-node-is.html#types-of-storage-nodes[Types of Storage Nodes] for more information about metadata-only Storage Nodes.
+
