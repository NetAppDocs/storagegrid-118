 ...
 ** *Storage Type* (Storage Nodes only): Specify that a new Storage Node be used exclusively for metadata. The options are *Objects and metadata* and *Metadata only*.
+
NOTE: When installing a grid with metadata-only nodes, the grid must also contain a minimum of nodes for object storage. For a single-site grid, at least two Storage Nodes are configured for objects and metadata. For a multi-site grid, at least one Storage Node per site are configured for objects and metadata.
...
+
For additional information, see the https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/index.html[Quick start for hardware installation^] to locate instructions for your appliance.
...
Rebooting can take several minutes.
 ...
 .. Confirm that you can configure the appropriate networks. They should already be populated with the information you provided on the IP Configuration page of the Appliance Installer.
+
To learn how to install StorageGRID appliances, see the https://review.docs.netapp.com/us-en/storagegrid-appliances_main/installconfig/index.html[Quick start for hardware installation^] to locate instructions for your appliance.
