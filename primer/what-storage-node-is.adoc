...
All Storage Nodes that were installed before StorageGRID 11.8 store both objects and the metadata for those objects. Starting in StorageGRID 11.8, you can choose the Storage Node type when you install a new node, as follows:
...
As required, you can specify that a new Storage Node be used exclusively for metadata.
+
TIP: You can only select the Storage Node type when installing the node initially. You can't change the type after the node has been installed.
