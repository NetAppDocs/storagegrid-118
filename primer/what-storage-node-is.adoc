...
You can specify that a new software-based Storage Node be used to store only metadata.
+
NOTE: You can only select the Storage Node type when initially installing the software-based node. You can't change the type after the node has been installed.
