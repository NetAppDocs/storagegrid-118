...
.. Select *Create bucket*.
+
NOTE: Don't select *Enable object versioning* for the FabricPool bucket. Similarly, don't edit a FabricPool bucket to use *Available* or a non-default consistency control. The recommended bucket consistency control for FabricPool buckets is *Read-after-new-write*, which is the default setting for a new bucket. 
