...
When you use the FabricPool setup wizard, the wizard automatically creates a new ILM rule for each S3 bucket you create and adds that rule to an inactive policy. You are prompted to activate the policy by assigning it to the default ILM policy tag. The automatically created rule follows the recommended best practices: it uses 2+1 erasure coding at a single site.
