...
+
NOTE: An ILM rule can't use both a *User defined creation time* for the Reference time and the Balanced or Strict options for Ingest Behavior. An error is returned when the ILM rule is created.
...
IMPORTANT: As a general best practice, always include these headers within `CanonicalHeaders` to ensure they are verified; however, if you exclude these headers, StorageGRID does not return an error.
