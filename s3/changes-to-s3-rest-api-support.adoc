...
|11.8
|
* Added support for `s3:ExistingObjectTag/<tag-key>` for existing objects that have the specific tag key and value.
* Added support for `s3:RequestObjectTag/<tag-key>`, which requires a specific tag and key value when the object request includes tagging.
