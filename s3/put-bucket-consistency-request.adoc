...
summary: 'The PUT Bucket consistency request allows you to specify the consistency setting to apply to operations performed on a bucket.'
---
...
[.lead]
The PUT Bucket consistency request allows you to specify the consistency setting to apply to operations performed on a bucket.
...
*Note:* In general, you should use the "`Read-after-new-write`" consistency setting. If requests aren't working correctly, change the application client behavior if possible. Or, configure the client to specify the consistency setting for each API request. Set the consistency setting at the bucket level only as a last resort.
