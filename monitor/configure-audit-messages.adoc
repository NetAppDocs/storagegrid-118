...
[.lead]
You can configure a number of settings related to audit messages. You can adjust the number of audit messages recorded; define any HTTP request headers you want to include in client read and write audit messages; and specify where audit logs, security event logs, and StorageGRID software logs are sent.
...
* If you plan to configure an external syslog server using TLS or RELP/TLS protocol, you have the following certificates:
...
All StorageGRID nodes generate audit messages and logs to track system activity and events. By default, audit information is sent to the audit log on Admin Nodes.
...
== Define HTTP request headers (optional)
...
== [[use-external-syslog-server]]Use external syslog server
