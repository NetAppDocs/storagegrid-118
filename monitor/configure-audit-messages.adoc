...
[NOTE]
====
StorageGRID defaults to local node audit destinations and stores the audit information in `/var/local/log/localaudit.log`. If you upgrade your appliance from StorageGRID 11.7, the audit messages continue to default to `/var/local/audit/export/audit.log` on the Admin Node, and security event logs and application logs to the local nodes.
