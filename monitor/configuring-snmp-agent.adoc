...
The authoritative engine ID is a 10-byte to 64-byte hex value, with no spaces or punctuation. This value is required for USM users that will be selected in trap destinations for informs. This value is not allowed for USM users that will be selected in trap destinations for traps.
